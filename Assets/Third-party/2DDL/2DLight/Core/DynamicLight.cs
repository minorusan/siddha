namespace DynamicLight2D
{
    ...
    public class DynamicLight : MonoBehaviour
    {
        ...
        private class Collider
        {
            ...
            internal Vector2[] worldPoints;
            internal Vector2 tempWorldPoint;
            ...
            internal Collider(CasterCollider co)
            {
                collider = co;
                //Debug.Log(collider);
                worldPoints = new Vector2[collider.TotalPointsCount];
            ...
            internal void UpdateWorldPoints(Vector3 lightPos, float lightRadius, int Layer, ref bool forceToUpdate)
            {
                ...
                int colliderGOLayer = collider.transform.gameObject.layer;
                int pointsCount = collider.points.Length;
                for (int i = 0; i < pointsCount; i++)
                {
                    //tempWorldPoint = collider.transform.TransformPoint(collider.points[i]);
                    tempWorldPoint = collider.worldPoints[i];

                    if (((1 << colliderGOLayer) & Layer) != 0)
                    {
                ...
                {                  // ...and for ever vertex we have of each mesh filter...
                    ...
                    if (_ray)
                    {
                        ...
                        if (notifyReachedEventUsed)
                        {
                            ...
                            {
                                if (Vector3.Angle(lightObjectTM.InverseTransformPoint(pos), Vector3.up) < RangeLightAngle * .5f)
                                {   // Light angle restriction
                    ...
                    if (360 != Mathf.RoundToInt(RangeLightAngle))
                    {
                        if (Vector3.Angle(pos, Vector3.up) < RangeLightAngle * .5f)
                        {   // Light angle restriction
                            if ((pos).sqrMagnitude <= LightRadius * LightRadius)
                            {
                                obj.Add(pos, endpoint, angle); // tempVerts.Add(v); // expensive
                                if (Debugging == true)
                                    UnityEngine.Debug.DrawLine(lightObjectTM.position, lightObjectTM.TransformPoint(pos), Color.white);
                            }
                        }
                    }
                    else
                ...
                if (obj.Count > 0)
                {
                    ...
                    for (int r = 0; r < endPointLimit; r++)
                    {
                        ...
                        if (r == 0)
                        {
                            ...
                            if (debugLines)
                                UnityEngine.Debug.DrawLine(fromCast, transform.position, Color.yellow);
                        ...
                        else if (r == 1)
                        {
                            ...
                            if (debugLines)
                                UnityEngine.Debug.DrawLine(fromCast, transform.position, Color.red);
                        ...
                        if (isEndpoint == true)
                        {
                            ...
                            if (_ray)
                            {
                                ...
                                if (((Vector2)gameObject.transform.position - hitp).sqrMagnitude < (mag * mag))
                                {
                                    ...
                                    if (notifyGameObjectsReached)
                                    {
                                        ...
                                        {
                                            if (Vector3.Angle(transform.InverseTransformPoint(hitp), Vector3.up) < RangeAngle * .5f)
                                            {   // Light angle restriction
                                                if (_ray.collider.gameObject.transform.parent)
                                                {
                            ...
                            else
                            {
                                ...
                                if (debugLines == true)
                                    ...
                                    UnityEngine.Debug.DrawLine(new Vector3(from.x, from.y, transform.position.z), new Vector3(hitp.x, hitp.y, transform.position.z), Color.blue);
                            ...
                            if (debugLines == true && _ray)
                                UnityEngine.Debug.DrawLine(new Vector3(fromCast.x, fromCast.y, transform.position.z), new Vector3(hitp.x, hitp.y, transform.position.z), Color.green);
            ...
            for (int i = 0; i < _allVerts.Count(); i++)
            {
                ...
                for (int j = 0; j < _allVerts.Count(); j++)
                {
                    ...
                    if (one.angle >= (two.angle - _rangeAngleComparative) && one.angle <= (two.angle + _rangeAngleComparative))
                    {
                        ...
                        if (one.endpoint == true | two.endpoint == true)
                        { // if any is endpoint
        ...
        void drawLinePerVertex()
        {
            for (int i = 0; i < _allVerts.Count; i++)
            {
                ...
                {
                    UnityEngine.Debug.DrawLine(_allVerts[i].pos, _allVerts[i + 1].pos, new Color(i * 0.02f, i * 0.02f, i * 0.02f));
                }
                else
                {
                    UnityEngine.Debug.DrawLine(_allVerts[i].pos, _allVerts[0].pos, new Color(i * 0.02f, i * 0.02f, i * 0.02f));
                }
            }
        }
        ...
        private void GenerateLightVectors()
        {
            ...
            for (int i = 0; i <= Segments; i++)
            {
                ...
                if (debugLines)
                    UnityEngine.Debug.DrawRay(transform.position, (transform.TransformPoint(v.pos) - transform.position), Color.grey);
